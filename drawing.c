...
void drawBoard() {
	const ALLEGRO_COLOR col = al_map_rgb(255, 100, 0);
...
void drawBall(const Ball * const ball) {
	const ALLEGRO_COLOR fill    = al_map_rgb(255, 0, 0);
	const ALLEGRO_COLOR outline = al_map_rgb(255, 255, 255);
	al_draw_filled_circle(ball->xpos, ball->ypos, ball->radius, fill);
	al_draw_circle(ball->xpos, ball->ypos, ball->radius, outline, 5);
